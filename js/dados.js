...
function salvar(event, collection) {
    ...
    else if (document.getElementById('salario').value === '') { alert('⚠ É obrigatório informar o salário!') }
    else if (document.getElementById('setor').value === '') { alert('⚠ É obrigatório informar o setor') }
...
function obtemDados(collection) {
    ...
    firebase.database().ref(collection).on('value', (snapshot) => {
        ...
        snapshot.forEach(item => {
            ...
            novalinha.insertCell().textContent = item.val().setor
            ...
            novalinha.insertCell().innerHTML =
                `
            <button class ='btn btn-danger' title='Remove o registro corrente' onclick=remover('${db}','${id}')>🗑 </button>
            <button class ='btn btn-secondary' title='Edita o registro corrente' onclick=carregaDadosAlteracao('${db}','${id}')>✏ </button>
            `
        })
        let rodape = tabela.insertRow()
        rodape.className = 'table-secondary'
...
/**
 * remover
 * Remove os dados da colection a partir do id informado
    ...
    firebase.database().ref(db).on('value', (snapshot) => {
        snapshot.forEach(item => {
            if (item.ref.path.pieces_[1] === id) {
                ...
                document.getElementById('setor').value = item.val().setor
                //campo sexo
